...
//    private void MqTask() {
//        try {
//            log.info("MyDefinedTask mq消息队列消费线程初始化开始！......");
...
//
//            log.info("MyDefinedTask 获取Kafka消息消费数据 test1Consumer:{}",test1Consumer);
//
...
//
//            log.info("MyDefinedTask 获取Kafka消息消费topic 分片数据 test1PartitionInfos:{}",test1PartitionInfos);
////            log.info("获取Kafka消息消费topic 分片数据 test2PartitionInfos:{}",test2PartitionInfos);
//
...
//
//                log.info("MyDefinedTask 当前时间：   " + DateUtil.getDate(DateUtil.DATE_FORMATE_YYYYMMDDHHMMSS) +"");
//                log.info("MyDefinedTask 偏移消费时间段：" + sd.format(calendar.getTime()) + " 00:00:00" +  " 至 " + sd.format(calendar.getTime()) + " 23:59:59" );
//
...
//                    test1EndTimestampsToSearch.put(new TopicPartition(n.topic(), n.partition()), DateUtil.getLastDayStartTimeStamp(10));
//                });
//
...
//
//                log.info("MyDefinedTask 开始设置各分区初始偏移量！......");
...
//                log.info("MyDefinedTask 设置各分区初始偏移量完毕！......");
//
//
...
////                    test2TopicPartitions.add(new TopicPartition(n.topic(), n.partition()));
////                });
//                log.info("MyDefinedTask isSetOffsetTime = " + mqConfig.getOffset() + "消费策略 = " + mqConfig.getAutoOffsetReset() );
//                test1Consumer.assign(test1TopicPartitions);
////                test2Consumer.assign(test2TopicPartitions);
//            }else {
//                log.error("MyDefinedTask mq消息参数配置有误，请检查配置文件！");
//                System.exit(-1);
//            }
...
//            log.info("MyDefinedTask mq消息队列消费线程初始化完成！......");
//        }catch (Exception e){
//            e.printStackTrace();
//            log.error("MyDefinedTask mq消息队列消费线程初始化失败！......" + e.getMessage());
//            System.exit(-1);
//        }
//    }
//
